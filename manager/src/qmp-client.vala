...
public class QmpClient : GLib.Object {
    ...
    public QmpClient(string socket_path) {
        ...
        this.pending = new GLib.HashTable<uint?, PendingCommand> (GLib.int_hash, GLib.int_equal);
    }
    ...
    public new async bool connect(GLib.Cancellable? cancellable = null) throws GLib.Error {
        ...
        try {
            ...
            input_stream = new GLib.DataInputStream(connection.get_input_stream());
    ...
    private async void read_response() {
        while (connection != null && !connection.is_closed()) {
            try {
                var line = yield input_stream.read_line_async (GLib.Priority.DEFAULT, null);
    ...
    private void handle_greeting(Json.Object obj) {
        ...
        message("QmpClient: QEMU %s.%s.%s QMP greeting received", major.to_string(), minor.to_string(), micro.to_string());
    ...
    private void handle_response(Json.Object obj) {
        ...
        if (obj.has_member("error")) {
            ...
            warning("QmpClient: command error: %s", error.get_string_member("desc"));
        }
    }
