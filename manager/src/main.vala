...
public class Qemu98Manager : Gtk.Application {
    ...
    public Qemu98Manager() {
        Object(application_id: "com.qemu98.manager", flags: ApplicationFlags.DEFAULT_FLAGS);
    }
    ...
    protected override void activate() {
        if (main_window == null) {
            ...
            controllers = new GLib.HashTable<string, VmController>(GLib.str_hash, GLib.str_equal);
            create_main_window();
    ...
    private void update_main_page(VmController.VmState state) {
        if (state == VmController.VmState.RUNNING || state == VmController.VmState.PAUSED) {
            main_stack.visible_child = runtime_notebook;
        }
        else if (state == VmController.VmState.STOPPED || state == VmController.VmState.ERROR) {
    ...
    private void update_action_states() {
        ...
        if (has_vm && controllers.contains(current_vm)) {
            ...
            is_running = c.state == VmController.VmState.RUNNING ||c.state == VmController.VmState.PAUSED ||c.state == VmController.VmState.STARTING;
        }
    ...
    private void on_vm_activated(string vm_name) {
        ...
        else if (ctrl.state == VmController.VmState.RUNNING || ctrl.state == VmController.VmState.PAUSED) {
            ctrl.stop();
        }
    }
