...
public class VmController : GLib.Object {
    ...
    public signal void snapshot_operation_complete(string op, bool success, string message);

    /** Emitted when a media change completes. */
    public signal void media_operation_complete(string device, bool success, string message);
    ...
    private void connect_qmp_with_retry() {
        retry_connect_source = GLib.Timeout.add(200, () => {
            ...
            var socket_file = GLib.File.new_for_path(process_manager.qmp_socket_path);
            ...
            qmp_client.connect.begin(null, (obj, res) => {
                try {
                    ...
                    warning("VmController[%s]: QMP connect failed: %s", vm_name, e.message);
    ...
    public void delete_snapshot(string name) {
        if (!qmp_connected) {
            snapshot_operation_complete("delete", false, "QMP not connected");
            return;
        }
        ...
        snapshot_operation_complete("delete", true, @"Snapshot '$name' deleted");
    }

    /** Restore a snapshot via QMP loadvm. */
    public void restore_snapshot(string name) {
        if (!qmp_connected) {
            snapshot_operation_complete("restore", false, "QMP not connected");
            return;
        }
        ...
        snapshot_operation_complete("restore", true, @"Snapshot '$name' restored");
    }
    ...
    public void change_media(string device_id, string file_path) {
        ...
        message("VmController[%s]: media changed on %s → %s", vm_name, device_id, file_path);
        media_operation_complete(device_id, true, @"Media changed to $(GLib.Path.get_basename(file_path))");
    }

    /** Eject media from a block device via QMP eject. */
    public void eject_media(string device_id) {
        if (!qmp_connected) {
            media_operation_complete(device_id, false, "QMP not connected");
            return;
        }
        ...
        media_operation_complete(device_id, true, "Media ejected");
    }
